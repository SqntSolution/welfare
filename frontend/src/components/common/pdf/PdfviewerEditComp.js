    ...
    return (
        <>
            ...
            <Row style={{ width: '100%', padding: 5, border: '1px solid #d1defc', }}>
                ...
                <Col span={24}>
                    <PdfviewerComp  {...props} detailId={detailId} path={path} isEdit={true} />
                </Col>
            </Row>
        </>
    )
...
const FileUploadArea = ({ initialFile: initialFile, setValidFile: setValidFile, t: t }) => {
        ...
        <>
            <div>
                <FileUpload readOnly={false} initialFileList={initialFile == null ? [] : [initialFile]} setValidFileList={list => setValidFile(list?.[0])} maxCount={1}
                    etcDescription={
                        <>
                            <FilePdfOutlined style={{ fontSize: '2em' }} />
                            <SUText16 style={{ marginTop: '0.5em' }}>pdf 파일을 마우스로 끌어오거나 여기를 클릭하여 첨부하세요.</SUText16>
                        </>
                    }
                    height='auto'
                />
