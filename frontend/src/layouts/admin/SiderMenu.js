	...
	const menuItems = () => {
		const list = [
			...
			getItem('고객센터', 'customer-cscenter', <CustomerServiceOutlined />,
				[
					getItem(<NavLink to='/admin/notice'>공지사항</NavLink>, '/admin/notice', <NotificationOutlined />),
					getItem(<NavLink to='/admin/faq'>질문모음</NavLink>, '/admin/faq', <QuestionCircleOutlined />),
					getItem(<NavLink to='/admin/qna'>문의하기</NavLink>, '/admin/qna', <QuestionOutlined />),
					getItem(<NavLink to='/admin/release'>보도자료</NavLink>, '/admin/release', <ReadOutlined />),
				]
			),
			...
			getItem(<NavLink to='/admin/code/common'>코드</NavLink>, '/admin/code/common', <CodeOutlined />),

			// getItem('테스트 페이지', 'test-page', <PlayCircleOutlined />,
			// 	[
			// 		getItem(<NavLink to='/admin/test/user'>유저 관리</NavLink>, '/admin/test/user', <UnlockOutlined />),
			// 	]
			// ),
		];
