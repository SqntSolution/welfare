...
import { Breadcrumb, Button, Col, Divider, Flex, Form, Input, Popconfirm, Radio, Row, Select, Space, Typography } from 'antd';
import { CustomAdminTitle, SubTitleDivider } from 'components/common/CustomComps';
...
const AdminRefCodeRegistration = () => {
    ...
    const getCodeGroupDetail = async () => {
        ...
        return await AXIOS.get(`/api/v1/admin/ref/${id}`)
            .then(async (resp) => {
    ...
    const getCommonCode = async () => {
        return AXIOS.get(`/api/v1/common/code-render/GROUP_TYPE`)
            .then(async (resp) => {
    ...
    const createCodeGroup = async () => {
        ...
        return await AXIOS.post(`/api/v1/admin/ref`, params)
            .then(async (resp) => {
                info('저장되었습니다.')
                navigate(-1);
                return resp;
    ...
    const updateCodeGroup = async () => {
        ...
        return await AXIOS.post(`/api/v1/admin/ref/${id}`, params)
            .then(async (resp) => {
                info('저장되었습니다.')
                navigate(-1);
                return resp;
    ...
    const deleteCodeGroup = async () => {
        ...
        return await AXIOS.delete(`/api/v1/admin/ref/${id}`)
            .then(async (resp) => {
                info('삭제되었습니다.')
                navigate(-1);
                return resp;
    ...
    const makeParam = async () => {
        form.setFieldValue('refList', form.getFieldValue('refList').map((d, i) => ({ ...d, seq: i + 1 })))
    ...
    const validateInput = async () => {
        ...
        if (isEmptyCheck(form.getFieldValue('groupCode')))
            return '그룹 코드 ID를 입력해주세요.';

        if (isEmptyCheck(form.getFieldValue('groupName')))
            return '그룹 코드 명을 입력해주세요.';

        if (isEmptyCheck(form.getFieldValue('refList')))
            return '코드 항목을 하나 이상 입력해주세요';

        if (isEmptyCheck(form.getFieldValue('groupType')))
            return '그룹 타입을 선택해주세요.';

        form.getFieldValue('refList').forEach((d1, i1, arr) => {
        ...
        <>
            ...
            <InnerAdminDiv>
                <Flex justify="space-between" align="end">
                    <Typography.Title level={4}>참조 코드</Typography.Title>
                </Flex>
                ...
                <Form form={form} labelCol={{ span: 4 }} >
                    ...
                    <StyledFormItem label="그룹 타입" name='groupType' required rules={[{ required: true, message: '그룹 타입을 선택해주세요.' }]}>
                        <Select options={commonCode} placeholder="그룹 타입을 선택해주세요." />
                    </StyledFormItem >
                    <Divider />
                    <Row style={{ width: '50%', }}>
                        <Form.List name='refList' noStyle={true}>
...
export default AdminRefCodeRegistration;