...
const AdminRefCodeList = () => {
    ...
    const getCodeGroupList = async () => {
        ...
        return await AXIOS.get("/api/v1/admin/ref")
            .then(async (resp) => {
                setData(resp?.data)
                return resp;
            })
    ...
    const columns = [
        ...
        { title: '등록 코드', dataIndex: 'refCount', key: 'refCount', ellipsis: true, width: '8%', },
        {
            title: '수정', key: 'groupCode', width: '8%',
            render: (_, record) => (
                <StyledTableButton type='link' onClick={() => navigate(`/admin/code/ref/edit/${record.id}`)}>수정</StyledTableButton>
            )
        },
        {
    ...
    return (
        <>
            ...
            <Flex justify="space-between" align="end" style={{ marginBottom: 24 }}>
                <Typography.Title level={4} style={{ margin: 0 }}>참조 코드</Typography.Title>
                <Button type='primary' size='large' style={{ width: 114 }} onClick={() => navigate('/admin/code/ref/new')}>추가</Button>
            </Flex>
            <StyledAdminTable
                dataSource={data ?? []}
                columns={columns ?? []}
...
export default AdminRefCodeList;